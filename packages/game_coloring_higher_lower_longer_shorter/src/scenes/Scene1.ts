...
export default class Scene1 extends Phaser.Scene {
    ...
    shutdown() {
        ...
        // KHÔNG STOP UI SCENE ĐỂ GIỮ BANNER
    ...
    private handlePartComplete(
        ...
        if (this.finishedParts.size >= this.totalParts) {
            ...
            // --- GAME HUB COMPLETE ---
            // game.finalizeAttempt(); // REMOVED: Only finalize at EndGame
            sdk.requestSave({
                score: this.score,
                levelIndex: 0,
            });
            ...
            // Xóa UI (Nút màu & Banner) -> BỎ THEO YÊU CẦU: GIỮ BANNER
            const uiScene = this.scene.get(SceneKeys.UI) as any;
            if (uiScene) {
    ...
    private playIntroSequence() {
        ...
        playVoiceLocked(null, AudioKeys.VoiceIntroS1);
