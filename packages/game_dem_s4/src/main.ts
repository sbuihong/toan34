...
configureSdkContext({
  fallback: {
    ...
    let isResetting = false; // Flag to prevent double-click
    ...
    function attachResetHandler() {
        ...
        if (resetBtn) {
            ...
            const oldHandler = (resetBtn as any).__resetHandler;
            if (oldHandler) {
                resetBtn.removeEventListener('click', oldHandler);
            }
            ...
            const resetHandler = async () => {
                ...
                if (window.gameScene && window.gameScene.scene) {
                     ...
                     // Finish voice session nếu có
                     if (activeScene && activeScene.voiceManager) {
                         console.log("[Reset] Finishing voice session before restart...");
                         try {
                            await activeScene.voiceManager.finishSession(3, true); // 3 levels, isUnload=true
                         } catch(err) {
                             console.error("Failed to finish voice session on reset:", err);
                         }
                     }
                    ...
                    // Reset flag sau khi restart xong
                    setTimeout(() => {
                        isResetting = false;
                    }, 1000);
                } else {
            ...
            // Store handler reference để remove sau này
            (resetBtn as any).__resetHandler = resetHandler;
