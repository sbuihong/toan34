...
export default class Scene1 extends Phaser.Scene {
    ...
    init(data?: any): void {
        ...
        // SDK initialization - 4 levels (khớp với config)
        game.setTotal(4);
    ...
    private loadLevel(levelId: number, playInstruction: boolean = false): void {
        ...
        if (!configData?.levels) {
            console.error("Invalid config structure - missing levels");
            return;
        }
        ...
        if (!imageConfig) {
            console.error(`Level ${levelId} missing image config`);
            return;
        }
        ...
        const x = GameUtils.pctX(this, imageConfig.x_pct || 0.5);
        const y = GameUtils.pctY(this, imageConfig.y_pct || 0.5);
        const img = this.add.image(x, y, imageConfig.textureKey)
            .setScale(imageConfig.scale || 0.65);
    ...
    private handleResult(result: any): void {
        ...
        sdk.progress({ levelIndex, total: 4, score: result.score });
     ...
     * Level 2 pass -> Load Level 3
     * Level 3 pass -> Load Level 4
     * Level 4 pass -> End Game
    ...
    private handleLevelPass(): void {
        ...
        if (this.currentLevelId === 1) {
            ...
            // Level 2 pass → Chuyển sang Level 3
            this.time.delayedCall(DELAY, () => {
                // Hide popup trước khi chuyển level
                const uiScene = this.scene.get(SceneKeys.UI) as any;
                if (uiScene?.hideScorePopup) uiScene.hideScorePopup();
            ...
            // Level 3 pass → Chuyển sang Level 4
            this.time.delayedCall(DELAY, () => {
                // Hide popup trước khi chuyển level
                const uiScene = this.scene.get(SceneKeys.UI) as any;
                if (uiScene?.hideScorePopup) uiScene.hideScorePopup();
                ...
                this.loadLevel(4, true); // Play instruction cho level 4
                game.startQuestionTimer();
            });
        } else if (this.currentLevelId === 4) {
            // Level 4 pass → End Game
            this.time.delayedCall(DELAY, () => {
                this.voiceManager.finishSession(4);
