...
export class VoiceManager {
    ...
    async submitAudio(
        ...
        try {
            ...
            // Tạo unique key cho result để check duplicate
            // Format: sessionId-submissionCount-globalIndex
            const resultKey = `${this.voiceHelper.sessionId}-${this.state.submissionCount}-${globalIndex}`;
            
            // Check nếu result này đã được xử lý
            if (this.processedResults.has(resultKey)) {
                console.warn(`[VoiceManager] Duplicate result detected - ignoring`, resultKey);
                return result; // Return result nhưng không trigger callback
            }
            
            // Đánh dấu result đã được xử lý
    ...
    async finishSession(totalLevels: number = 1, isUnload: boolean = false): Promise<any | null> {
        ...
        try {
            ...
            // Clear processed results khi finish session
