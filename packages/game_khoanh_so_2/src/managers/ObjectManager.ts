...
export class ObjectManager {
    ...
    public spawnObjectsFromConfig(configData: any) {
        ...
        const targets = configData.targets || configData.images; // Support both new and old structure for compatibility if needed? 
        // User requested "change", so we focus on new.
        ...
        if (Array.isArray(targets)) {
            targets.forEach((def: any, index: number) => {
                ...
                // Tính toán toạ độ
                const x = (def.x_pct !== undefined) ? def.x_pct * width : (def.baseX_pct * width);
                const y = (def.y_pct !== undefined) ? def.y_pct * height : (def.baseY_pct * height);
                
                if (def.textureKey) {
                    // Cấu trúc cũ: Image thật
                    const img = this.scene.add.image(x, y, def.textureKey);
                    if (def.baseScale) img.setScale(def.baseScale);
                    obj = img; // Cast later
                } else {
                    // Cấu trúc mới: Toạ độ + Radius -> Tạo Invisible Helper Image (or Zone)
                    // Dùng Image với texture trong suốt hoặc 'dic' (nếu có) nhưng setAlpha(0.01) để có size.
                    // Phaser Zone không có 'getOverlapPercentage' logic dựa trên texture pixels/bounds giống Image mặc định?
                    // Thực ra logic getOverlapPercentage của mình dùng getBounds(), Zone cũng có getBounds().
                    // Nhưng để an toàn với logic hiện tại (expects Image features like setTint), 
                    // ta dùng một Placeholder Image (ví dụ: 'number' hoặc transparent rect).
                    
                    // Tạo một Circle Graphics làm texture 'hitarea' ảo nếu cần, hoặc dùng 1 ảnh có sẵn và setAlpha(0.01).
                    // Để đơn giản và visual feedback, ta có thể dùng 1 ảnh tròn mờ.
                    
                    // For now, use a dummy texture found in keys (e.g. 'ball' or 'number') but make it invisible.
                    // Or ideally create a transparent texture on fly.
                    
                    // Hack: Use 'number' or any small texture, scale it to match radius.
                    const dummyKey = 'ball'; // TextureKeys.S1_Ball
                    const img = this.scene.add.image(x, y, dummyKey);
                    
                    // Set size theo radius config
                    if (def.radius) {
                        const currentW = img.width;
                        const scale = (def.radius * 2) / currentW;
                        img.setScale(scale);
                    } else {
                        img.setScale(0.5);
                    }
                    
                    img.setAlpha(0.01); // Gần như tàng hình nhưng vẫn tương tác được
                    obj = img;
                }

                // Lưu metadata
                obj.setData('role', def.role || 'wrong'); 
                obj.setData('id', def.id !== undefined ? def.id : index);

                this.objects.push(obj as Phaser.GameObjects.Image);

                // 3. Render Circle Outline if 'activity' is true
                if (def.activity) {
                     const graphics = this.scene.add.graphics();
                     graphics.lineStyle(5, 0xff0000); // Red outline, 2px
                     
                     // Get radius from config (same as used for scaling)
                     let radius = def.radius;
                     if (!radius) {
                         // Fallback geometry if radius not explicit (though our logic uses it)
                         radius = (Math.max(obj.displayWidth, obj.displayHeight) / 2);
                     }
                     ...
                     obj.setData('visualOutline', graphics);
                }
            });
        }
