...
export default class Scene4 extends Phaser.Scene {
    ...
    private createUI() {
        ...
        // Scene 4 Banner
        const bannerHeight = this.textures.get(TextureKeys.S4_Banner).getSourceImage().height * 0.7; // Scale 0.7
    ...
    private createLevel() {
        // Load Scene 4 Config
        const data = this.cache.json.get(DataKeys.LevelS4Config);
        if (data) {
